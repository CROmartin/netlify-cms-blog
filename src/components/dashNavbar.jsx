...
const Navbar = () => {
  ...
  //when you put it in global context delete this
  ...
  if (isBrowser) {
    if (
      window.location.pathname !== "/app/checkout" ||
      window.location.pathname !== "/app/checkout-new1" ||
      window.location.pathname !== "/app/checkout/" ||
      window.location.pathname !== "/app/checkout-new1/"
    ) {
      specialMenu = true
  ...
  //Global context Hook
  const { user, authNumber } = useContext(GlobalAuthContext)
      ...
      <NavbarStyle nav={nav}>
        <ul>
          <li className="different">
            {specialMenu && (
              <Link to="/app/manage-subscription" className="start">
                Checkout
              </Link>
            )}
          </li>
